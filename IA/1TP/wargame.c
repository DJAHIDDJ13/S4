...
int f_eval(Pion* jeu, int joueur) {
	int dist_diff = 0;
	int val_diff = f_valeur(jeu, joueur) - f_valeur(jeu, -joueur);
	for (int i = 0; i < NB_LIGNES; i++) {
		for (int j = 0; j < NB_COLONNES; j++) {
			int col = jeu[i*NB_COLONNES + j].couleur;
			int goal = (col == -1)? -1:10;
			if(col == joueur) {
				dist_diff += abs(goal - i);
			} else if(col == -joueur) {
				dist_diff -= abs(goal - i);
			}
		}
	}
	return val_diff * 50 + dist_diff;
}
...
int f_max(Pion* plateau_courant, int profondeur, int joueur);

//Fonction min trouve le minimum des noeuds fils
int f_min(Pion* plateau_courant, int profondeur, int joueur) {
	if(profondeur <= 0) {
				...
				for(int x=-1; x<=1; x++) {
					for(int y=-1; y<=1; y++) {
						...
						if(f_bouge_piece(plateau_suivant, i, j, voisinX, voisinY, -joueur) == 0) {
							int newval = f_max(plateau_suivant, profondeur-1, joueur);
							if(newval < minval) {
								minval = newval;
							}
			...
			if(plateau_courant[i*NB_COLONNES + j].couleur == joueur) {
				for(int x=-1; x<=1; x++) {
					for(int y=-1; y<=1; y++) {
						...
						if(f_bouge_piece(plateau_suivant, i, j, voisinX, voisinY, joueur) == 0) {
							int newval = f_min(plateau_suivant, profondeur-1, joueur);
							if(newval > maxval) {
								maxval = newval;
								fromX = i; fromY = j;
...
void f_IA(int joueur)
{
	...
	f_max(plateauDeJeu, PROF_MAX, joueur);
